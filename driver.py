...
def check_pn_containment_html(verifier, gen_report, sfc1, pn1, sfc2, pn2):
    ...
    sfc1_dot = os.path.join(OUTPUT_DIR, "sfc1.dot")
    sfc1_png = os.path.join(OUTPUT_DIR, "sfc1.png")

    pn1_dot = os.path.join(OUTPUT_DIR, "pn1.dot")
    pn1_png = os.path.join(OUTPUT_DIR, "pn1.png")

    sfc2_dot = os.path.join(OUTPUT_DIR, "sfc2.dot")
    sfc2_png = os.path.join(OUTPUT_DIR, "sfc2.png")

    pn2_dot = os.path.join(OUTPUT_DIR, "pn2.dot")
    pn2_png = os.path.join(OUTPUT_DIR, "pn2.png")

    gen_report.sfc_to_dot(sfc1, sfc1_dot)
    gen_report.dot_to_png(sfc1_dot, sfc1_png)

    gen_report.petrinet_to_dot(pn1, pn1_dot)
    gen_report.dot_to_png(pn1_dot, pn1_png)

    gen_report.sfc_to_dot(sfc2, sfc2_dot)
    gen_report.dot_to_png(sfc2_dot, sfc2_png)

    gen_report.petrinet_to_dot(pn2, pn2_dot)
    gen_report.dot_to_png(pn2_dot, pn2_png)

    img_paths = {
        "sfc1": gen_report.img_to_base64(sfc1_png),
        "pn1": gen_report.img_to_base64(pn1_png),
        "sfc2": gen_report.img_to_base64(sfc2_png),
        "pn2": gen_report.img_to_base64(pn2_png)
    }

    return gen_report.generate_containment_html_report(
        verifier.cutpoints1, verifier.cutpoints2,
        verifier.paths1, verifier.paths2,
        verifier.matches1, verifier.unmatched1,
        verifier.contained, img_paths
    )
...
def main():
    ...
    html_path = os.path.join(OUTPUT_DIR, "pn_containment_report.html")
    json_path = os.path.join(OUTPUT_DIR, "pn_containment_report.json")
